...
if __name__ == '__main__':
    ...
    if debugging:
        ...
        num_threads = 24
        args = [(X_train_without_bias, y_train, X_test_without_bias, y_test, amount_of_data_in_interval,  np.random.randint(max_integer_val), parameters) for i in range(num_threads)] 
        t1 = time.time()
        p = Pool(num_threads)
...
#%%        
    ...
    for result in results:
        for d in result:
            d_key = str(d)
            if d_key not in results_flatten:
                results_flatten[d_key] = {}
            for epsilon in result[d]:
                epsilon_key = str(epsilon) # json wants its key as string 
                if epsilon_key not in results_flatten[d_key]:
                    results_flatten[d_key][epsilon_key] = {}
                for n in result[d][epsilon]:
                    n_key = str(n) # json wants its key as string 
                    if n_key not in results_flatten[d_key][epsilon_key]:
                        results_flatten[d_key][epsilon_key][n_key] = {}
                        results_flatten[d_key][epsilon_key][n_key]['error_rate'] = [result[d][epsilon][n]['error_rate']]
                        #results_flatten[epsilon_key][n_key]['noise'] = result[epsilon][n]['noise']
                        results_flatten[d_key][epsilon_key][n_key]['noise_and_weights_magnitude'] = [result[d][epsilon][n]['noise_and_weights_magnitude']]
                        results_flatten[d_key][epsilon_key][n_key]['weights'] = [result[d][epsilon][n]['weights']]
                    else:
                        results_flatten[d_key][epsilon_key][n_key]['error_rate'].append(result[d][epsilon][n]['error_rate'])
                        #results_flatten[epsilon_key][n_key]['noise'] += result[epsilon][n]['noise']
                        results_flatten[d_key][epsilon_key][n_key]['noise_and_weights_magnitude'].append(result[d][epsilon][n]['noise_and_weights_magnitude'])
                        results_flatten[d_key][epsilon_key][n_key]['weights'].append(result[d][epsilon][n]['weights'])
                    
        
    # save the data as json and use it in the plot_results.py file
    file_name = 'results_dim.json'
...
#%%
    ...
    for i, objective_info in enumerate(objective_infos):
        for d in objective_info:
            d_key = str(d)
            if d_key not in objective_info_flatten:
                objective_info_flatten[d_key] = {}
            for  epsilon in objective_info[d]:
                if epsilon not in objective_info_flatten:
                    objective_info_flatten[d_key][epsilon] = {'objective': [], 'gradient': [], 'num_points': []}
                for j in range(len(objective_info[d][epsilon]['objective'])):
                    ...
                    if i == 0:
                        ...
                        objective_info_flatten[d_key][epsilon]['objective'].append([objective_info[d][epsilon]['objective'][j]])
                        objective_info_flatten[d_key][epsilon]['gradient'].append([objective_info[d][epsilon]['gradient'][j]])
                        objective_info_flatten[d_key][epsilon]['num_points'].append(objective_info[d][epsilon]['num_points'][j])
                    else:
                        objective_info_flatten[d_key][epsilon]['objective'][j].append(objective_info[d][epsilon]['objective'][j])
                        objective_info_flatten[d_key][epsilon]['gradient'][j].append(objective_info[d][epsilon]['gradient'][j])
                
            
    # save as json nad use it in plot_results.py file
    file_name = 'objective_info_dim.json'
    with open(file_name, 'w') as f:
